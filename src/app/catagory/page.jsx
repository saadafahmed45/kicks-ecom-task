...
const CategoryPage = () => {
  ...
  return (
    <section className="bg-black py-10 px-4">
      <div className=" mx-auto px-0 sm:px-8 lg:px-12">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-3xl sm:text-5xl font-extrabold text-white">CATEGORIES</h2>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 bg-white rounded-tl-[25px] rounded-br-[25px] ">
            {categories.slice(startIdx, startIdx + visibleCount).map(category => (
              <Link
                key={category.id}
                href={`/product?category=${encodeURIComponent(category.name)}`}
                className="bg-gray-100 rounded-tl-[25px] rounded-tr-none rounded-bl-none rounded-br-[25px] p-8 flex flex-col justify-between min-h-96 relative transition hover:shadow-2xl hover:scale-105 duration-300 cursor-pointer"
              >
                <img
                  src={category.image}
                  alt={category.name}
